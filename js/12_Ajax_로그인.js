...
$(function () {
  ...
  $("#로그인기능").click(function (e) {
    ...
    $.ajax({
      url: "../json/data.json", // json파일이 위치한 곳으로 url 주소 설정
      method: "GET",
      dataType: "json",
      success: function (data) {
        // case1. 무사히 url에 접속해서 데이터를 가져왔는데 데이터가 존재하지 않는 경우
        // case2. 데이터가 사용자가 작성한 정보와 일치하지 않아서 데이터를 가져올 수 없는 경우
        // case3. 데이터가 존재할 경우
        console.log("성공적으로 json에서 가져온 데이터 확인하기", data);
        // 만약 json에서 확인한 데이터와 소비자가 작성한 데이터가 일치한다면
        ...
        users[abc] -> users[admin]으로 변경되어 admin과 일치하는 ID를 검색
        */

        // users는 배열 형태
        // users.username (X) -> users[username] (O)
        if (data.users[userID].password === password) {
          $("#result").html(
