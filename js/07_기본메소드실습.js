$(document).ready(function () {
  ...
  $(".modal-content").click(function (e) {
    ...
    /*
    둘 다 modal 내부를 클릭했을 때 꺼짐 방지 설정

    방법1  : 조건부로 ~식으로 처리하겠다 같은 형식 설정 방법
    if (e.target === this) {
      $("#loginModal").fadeOut();
    }

    방법2  : 조건부 형식 설정 없이 modal 내부를 클릭했을 때 꺼짐 방지 설정
    e.stopPropagation(); : 특정 행동이 전달되지 않도록 설정
    */
    e.stopPropagation();
  });
